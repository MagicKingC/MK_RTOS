...
MK_RTOS_EXT mk_uint32 _MK_Highest_Prio_Index;/* 最高任务优先级的选项 */
...
typedef struct _MK_TASK_{
	...
	mk_bool 	is_auto_stck;	/* 是否自动分配任务栈 MK_FALSE:不自动分配 MK_TRUE:自动分配 */
	mk_uint32	task_stck_size; /* 是否任务栈大小，如果采用的是静态堆栈，该字段不必设置 */
